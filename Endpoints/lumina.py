...
def mqtt_on_message(client,userdata,msg):
    ...
    print(f"Luminosity: received {msg.payload.decode()} from {msg.topic} topic")

    json_msg=json.loads(msg.payload.decode())

    # Save received state
    db=get_db()
    db.execute(f"INSERT INTO luminosity(value) VALUES ({json_msg['value']})")
    db.commit()
